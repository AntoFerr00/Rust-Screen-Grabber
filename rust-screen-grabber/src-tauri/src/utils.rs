...
pub fn utils_dir() -> String {
    if !cfg!(target_os="windows") { env::var("HOME").unwrap() + "/.rust-screen-grabber" }
    else { env::var("APPDATA").unwrap() + "/.rust-screen-grabber" }
}
...
pub fn monitor_dialog(app: AppHandle) {
    MessageDialogBuilder::new("New Monitor Detected", "rust-screen-grabber detected a new monitor. Please Restart the application in order to re-index all monitors.")
