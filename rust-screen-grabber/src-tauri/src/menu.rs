...
pub fn create_context_menu() -> Menu {
    ...
    #[cfg(target_os = "macos")]
    let about = Submenu::new("Rust Screen Grabber",
                             Menu::new()
    ...
    let record = Submenu::new("Record",
                              Menu::new().add_item(CustomMenuItem::new("fullscreen_record".to_string(), "Fullscreen Record").accelerator(&hotkeys.fullscreen_record))
                                  ...
                                  .add_native_item(MenuItem::Separator)
                                  .add_item(CustomMenuItem::new("record_external_audio".to_string(), "Record External Audio").selected().accelerator(&hotkeys.record_external_audio))
