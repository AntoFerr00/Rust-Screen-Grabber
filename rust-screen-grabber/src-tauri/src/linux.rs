    ...
    if status.success() {
        ...
        if clipboard {
            if let Err(_err) = copy_to_clipboard(filename.to_string()) {
                return Response::new(Some(format!("Failed to copy on Clipboard. Rust Screen Grabber saved to {}", filename.to_string())), None);
            }
            else {
                fs::remove_file(filename.to_string()).unwrap();
                return Response::new(Some(format!("Rust Screen Grabber saved to Clipboard")), None);
            }
        } else {
            return Response::new(Some(format!("Rust Screen Grabber saved to {}", filename.to_string())), None);
        }
    }
    return Response::new(None, Some(format!("Rust Screen Grabber cancelled")));
}
